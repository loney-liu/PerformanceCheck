...
class DNS(object):
    ...
    def __init__(self):
        ...
        logging.info("Reading setIP.yml...\n")
        if os.path.exists("setIP.yml"):
            try:
                with open("setIP.yml", "r") as fh:
                    self._dnsservers = yaml.load(fh, Loader=yaml.FullLoader)
            except Exception as e:
                logging.info("Could not parse setIP.yml: %s" % e)
                return
        else:
            logging.error("Did not find setIP.yml. See README.md for details.")
            return
        if not self._dnsservers:
            logging.error("Settings dict is empty (bad \"setIP.yml\" file?), exiting.")
            return
    ...
    def _start(self):
        ...
        for s3_url in self._dnsservers.keys():
            logging.info("%s\n"%s3_url)
            for datacenter in self._dnsservers[s3_url].keys():
                logging.info("Datacenter: %s"%(datacenter))
                ip = self._dnsservers[s3_url][datacenter]
