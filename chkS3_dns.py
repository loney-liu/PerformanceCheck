...
class DNS(object):
    ...
    def __init__(self):
        ...
        logging.info("Reading setDNS.yml...\n")
        if os.path.exists("setDNS.yml"):
            try:
                with open("setDNS.yml", "r") as fh:
                    self._dnsservers = yaml.load(fh, Loader=yaml.FullLoader)
            except Exception as e:
                logging.info("Could not parse setDNS.yml: %s" % e)
                return
        else:
            logging.error("Did not find setDNS.yml. See README.md for details.")
            return
        if not self._dnsservers:
            logging.error("Settings dict is empty (bad \"setDNS.yml\" file?), exiting.")
            return
